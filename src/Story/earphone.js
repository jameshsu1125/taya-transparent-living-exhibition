...
import './earphone.less';
import Page0 from './earphone/page0';
import Page1 from './earphone/page1';
import Page2 from './earphone/page2';
import Page3 from './earphone/page3';

const { parseInt } = window;
const pageNumber = QueryString.get('page');
const queryIsVR = QueryString.get('vr');

const Earphone = (props) => {
	const {
		categroyName,
		setLoading,
	...
	useEffect(() => {
		new ImageOnload(container.current, { hideBeforeLoaded: true }).then(() => {
			...
			Click.add('#vr', () => {
				window.location.href = VR_URL.earphone;
			});
		});
		return () => {
			Click.remove('.return');
	...
	return (
		<div ref={container} className='Earphone'>
