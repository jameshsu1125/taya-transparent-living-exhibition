...
import { HASHTAG, STORY_EARPHONE_PAGE3, VR_URL } from '../../Setting/config';
...
const Page3 = (props) => {
	...
	useEffect(() => {
		...
		Click.add('#desktop_return', () => {
			...
			if (queryIsVR === '1') window.location.href = VR_URL.earphone;
			setTimeout(() => {
				if (device) back();
		...
		Click.add('#desktop_share', () => {
			...
			if (sliderSteps) redirect_uri += '?s=2';
			if (queryIsVR === '1') {
				redirect_uri = VR_URL.earphone;
			}
